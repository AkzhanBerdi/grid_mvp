    ...
    def calculate_fifo_profit(self, client_id: int) -> Dict:
        ...
        try:
            with sqlite3.connect(self.db_path) as conn:
                ...
                current_multiplier = (
                    1.0 + (max(total_profit, 0) / base_profit_threshold) * 0.1
                )
                ...
                return {
                    ...
                    "avg_profit_per_trade": round(
                        total_profit / max(len(trades), 1), 2
                    ),
            ...
            self.logger.error(
                f"Error calculating FIFO profit for client {client_id}: {e}"
            )
            return {
                "total_profit": 0.0,
                "total_trades": 0,
                ...
                if trade.side == "BUY":
                    ...
                    while buy_queue and trade.quantity > 0:
                        ...
                        if buy_trade.quantity <= trade.quantity:
                            ...
                            profit_percentage = (
                                profit / (buy_trade.price * quantity_matched)
                            ) * 100
