...
class AsyncClientRepository:
    ...
    async def update_client_status_async(
        self, telegram_id: int, grid_status: str = None, client_status: str = None
    ) -> bool:
...
class AsyncAnalytics:
            ...
            if result:
                ...
                return {
                    ...
                    "avg_profit_per_trade": round(
                        (total_sells - total_buys) / max(total_trades, 1), 2
                    ),
                }
    ...
    async def get_recent_trades_async(
        ...
        try:
            ...
            for query_name, times in self.query_times.items():
                ...
                if avg_time > 0.5:  # Queries taking more than 500ms on average
                    slow_tables.append(
                        {"query": query_name, "avg_time": round(avg_time, 3)}
                    )

            return {
                "database_size_mb": round(db_size / (1024 * 1024), 2),
                "table_info": [
                    {"name": row[0], "estimated_rows": row[1]} for row in tables_result
                ],
...
class HybridGridManager:
    ...
    async def place_order_and_record(self, order_data: Dict) -> bool:
        ...
        try:
            ...
            self.logger.info(
                f"✅ Order placed: {order_data['symbol']} {order_data['side']}"
            )
            return True
