...
class GridPerformanceCalculator:
    ...
    def calculate_grid_performance(self, client_id: int, days: int = 30) -> Dict:
        ...
        try:
            ...
            self.logger.info(f"📊 Performance calculated for client {client_id}: {performance['total_profit']:.2f}")
            return performance
    ...
    def _calculate_grid_metrics(self, trades: List[Dict]) -> Dict:
        ...
        return {
            ...
            'price_spread': avg_sell_price - avg_buy_price if avg_sell_price and avg_buy_price else 0
        }
    ...
    def _calculate_position_profits(self, trades: List[Dict]) -> Dict:
        ...
        # Performance rating
        performance_rating = self._calculate_performance_rating(total_profit, win_rate, len(trades))
    ...
    def _calculate_performance_rating(self, total_profit: float, win_rate: float, total_trades: int) -> str:
        ...
        profit_score = min(100, max(0, (total_profit + 100) / 2))  # Scale around break-even
        win_rate_score = win_rate
        volume_score = min(100, total_trades * 2)  # More trades = more data
        
        overall_score = (profit_score * 0.5) + (win_rate_score * 0.3) + (volume_score * 0.2)
    ...
    def calculate_trading_efficiency(self, client_id: int) -> float:
        ...
        try:
            ...
            consistency_score = min(100, 50 / max(1, avg_profit)) if avg_profit > 0 else 0
    ...
    def get_client_trade_stats(self, client_id: int) -> Dict:
        ...
        return {
            **performance,
            ...
            'market_adaptation_score': self.performance_calc.calculate_market_adaptation_score(client_id),
            'trading_efficiency_score': self.performance_calc.calculate_trading_efficiency(client_id)
        }
