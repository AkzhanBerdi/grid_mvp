...
class BaseClientHandler:
    ...
    async def setup_api_keys(self, query, client_id: int):
        """API key setup with Home button"""
    ...
    **Account Status:** {"✅ Ready" if client.can_start_grid() else "❌ Setup Required"}"""
    ...
    # ALTERNATIVE SIMPLER FIX - if you want to just handle the specific error:
    ...
    async def show_base_dashboard(
        ...
        """
        SIMPLE FIX: Just handle the coroutine error gracefully
        """
        try:
            # Try to get grid status, handle both async and sync cases
            grid_status = None
            try:
                ...
                self.logger.warning(f"Grid status error: {grid_error}")
                # Continue without grid status
                grid_status = {"active_grids": {}}
    ...
    **Quick Trading:** Type `ADA 1000` or `ETH 500`"""
    ...
    Use quick commands: `ADA 1000` or `ETH 500`"""
            ...
            try:
                if hasattr(update, "message") and update.message:
                    ...
                    await update.edit_message_text(fallback_message)
            except:
                pass  # Give up gracefully

    async def stop_all_grids(self, query, client_id: int):
        """Unified grid stopping - with Home button"""
    ...
    Please check manually or contact support."""
        ...
        keyboard = [
            [InlineKeyboardButton("🏠 Home", callback_data="home")]  # ✅ CHANGED
        ]
...
**Recommended:** Start with $500-1000 for best results"""
    ...
    async def handle_common_callbacks(self, query, client_id: int, action: str) -> bool:
        ...
        if action == "setup_api":
            ...
            return True
        elif action == "show_dashboard" or action == "home":
            # Go back to personalized /start instead of generic dashboard
    ...
    async def go_home(self, query, client_id: int):
        """Super simple home - just edit the current message"""
        try:
            client = self.client_repo.get_client(client_id)
