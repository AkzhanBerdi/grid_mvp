    ...
    def calculate_fifo_profit(self, client_id: int) -> Dict:
        ...
        try:
            with sqlite3.connect(self.db_path) as conn:
                ...
                avg_profit = total_profit / completed_cycles if completed_cycles > 0 else 0
                ...
                return {
                    ...
                    'profit_margin': round((total_profit / total_volume * 100), 3) if total_volume > 0 else 0
                }
    ...
    async def on_trade_executed(self, symbol: str, side: str, quantity: float, price: float):
        ...
        try:
            ...
            if estimated_trade_profit >= 1.0:  # $1+ profit
                await self._notify_significant_trade(symbol, estimated_trade_profit, current_stats)
...
🚀 Keep the momentum going! 💪"""
    ...
    async def _notify_significant_trade(self, symbol: str, profit: float, current_stats: Dict):
...
class FIFOMonitoringService:
    ...
    async def on_trade_executed(self, client_id: int, symbol: str, side: str, quantity: float, price: float):
        """Call this from your trade execution logic"""
        try:
            if client_id in self.monitors:
                await self.monitors[client_id].on_trade_executed(symbol, side, quantity, price)
        except Exception as e:
    ...
    async def send_profit_status(self, client_id: int):
        ...
        try:
            ...
            self.logger.error(f"Error sending profit status for client {client_id}: {e}")
