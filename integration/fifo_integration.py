...
class EnhancedTradeRepository:
    ...
    def get_client_trade_stats(self, client_id: int) -> dict:
        ...
        try:
            ...
            return {
                ...
                'next_order_size': Config.DEFAULT_ORDER_SIZE * fifo_metrics.current_multiplier,
    ...
    def _get_basic_stats(self, client_id: int) -> dict:
        ...
        try:
            ...
            return {'total_trades': 0, 'total_volume': 0.0, 'total_profit': 0.0, 'win_rate': 0.0, 'recent_trades': []}

    def log_trade_execution_with_fifo(self, client_id: int, symbol: str, side: str, 
        ...
        try:
            ...
            self.logger.info(f"Trade logged with FIFO update: {symbol} {side} {quantity} @ {price}")
        ...
        try:
            ...
            if client:
                ...
                if abs(client.order_size - new_order_size) > 1.0:
                    ...
                    self.logger.info(f"Updated client {client_id} order size to ${new_order_size:.2f} (multiplier: {new_multiplier:.2f}x)")
...
class EnhancedClientHandler:
    ...
    async def _show_performance(self, query, client_id):
        ...
        try:
            ...
            performance = await self.performance_monitor.monitor_client_performance(client_id)
            
            if 'error' in performance:
                await query.edit_message_text(f"❌ Error loading performance: {performance['error']}")
                return
   ...
   Profit Margin: {(metrics.total_realized_profit/metrics.total_volume*100):.3f}%"""
    ...
    async def handle_callback(self, update, context):
        ...
        if update.callback_query.data == "show_performance":
            await self._show_performance(update.callback_query, update.callback_query.from_user.id)
        else:
        ...
        while True:
            try:
                ...
                for client_id in active_clients[:10]:  # Show top 10 clients
                    try:
                        ...
                        performance = asyncio.run(
                            self.performance_monitor.monitor_client_performance(client_id)
                        )
                        ...
                        if 'error' not in performance:
                            ...
                            # Status indicators
                            profit_emoji = "🟢" if metrics.total_realized_profit > 0 else "🔴"
                            compound_emoji = "🚀" if metrics.current_multiplier > 1.0 else "⚪"
                            
                            print(f"{profit_emoji} Client {client_id}:")
                            print(f"   💰 Profit: ${metrics.total_realized_profit:>8.2f}")
                            print(f"   📊 Win Rate: {metrics.win_rate:>5.1f}%")
                            print(f"   {compound_emoji} Multiplier: {metrics.current_multiplier:>4.2f}x")
                ...
                if client_performances:
                    avg_win_rate = sum(p.win_rate for p in client_performances) / len(client_performances)
                    avg_multiplier = sum(p.current_multiplier for p in client_performances) / len(client_performances)
                    profitable_clients = len([p for p in client_performances if p.total_realized_profit > 0])
                    
                    print(f"Average Win Rate:        {avg_win_rate:>10.1f}%")
                    print(f"Average Multiplier:      {avg_multiplier:>10.2f}x")
                    print(f"Profitable Clients:      {profitable_clients:>10}/{len(client_performances)}")
...
class FIFOMigration:
    ...
    def migrate_to_fifo_system(self):
        ...
        try:
            ...
            if active_clients:
                ...
                print(f"   📊 Profit: ${metrics.total_realized_profit:.2f}, Trades: {metrics.total_trades}")
            else:
            ...
            print("      from integration.fifo_integration import EnhancedClientHandler")
            print("      from integration.fifo_integration import EnhancedMonitoringDashboard")
            print()
            print("   📝 Update grid_orchestrator.py trade logging:")
            print("      Use log_trade_execution_with_fifo() instead of log_trade_execution()")
            print()
    ...
    def generate_fifo_comparison_report(self, client_id: int):
        ...
        try:
            ...
            print(f"\n🔍 **FIFO ADVANTAGES:**")
            print(f"   ✅ Matches actual trading cycles")
            print(f"   ✅ Tax-compliant FIFO accounting")
            print(f"   ✅ Accurate profit per trade")
...
def main():
    ...
    parser.add_argument("--compare", type=int, metavar="CLIENT_ID", help="Compare profit calculations")
    parser.add_argument("--test", type=int, metavar="CLIENT_ID", help="Test FIFO tracking for client")
    parser.add_argument("--dashboard", action="store_true", help="Run enhanced dashboard")
