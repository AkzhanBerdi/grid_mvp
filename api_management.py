    ...
    def list_clients_with_api_keys(self):
        ...
        try:
            with sqlite3.connect(self.db_path) as conn:
                ...
                print("=" * 80)
                print(
                    f"{'ID':<12} {'Username':<15} {'Name':<15} {'API':<5} {'Secret':<6} {'Status':<10} {'Created'}"
                )
                print("-" * 80)
                ...
                for client in clients:
                    ...
                    print(
                        f"{telegram_id:<12} {username or 'N/A':<15} {first_name or 'N/A':<15} "
        ...
        try:
            with sqlite3.connect(self.db_path) as conn:
                if telegram_id:
                    ...
                    confirm = input(
                        f"⚠️  This will remove API keys from {count} clients. Continue? (yes/no): "
                    )
        ...
        try:
            ...
            if telegram_id:
                ...
                with sqlite3.connect(self.db_path) as conn:
                    ...
                    clients_to_check = [
                        self.client_repo.get_client(cid) for cid in client_ids
                    ]
            ...
            for client in clients_to_check:
                ...
                try:
                    ...
                    decrypted_secret = self.crypto_utils.decrypt(
                        client.binance_secret_key
                    )

                    if decrypted_api and decrypted_secret:
                        print(
                            f"  ✅ API keys valid (lengths: {len(decrypted_api)}, {len(decrypted_secret)})"
                        )
                    else:
        ...
        while True:
            ...
            if choice == "1":
                ...
                telegram_id_input = input(
                    "Enter Telegram ID (or press Enter for all): "
                ).strip()
                telegram_id = None
